...
class Lokasi_ujian extends Rest_Controller {
	
	var $kode_menu = "M002005";
	var $table = "ref_lokasi_ujian";
	var $menu;

	function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->model('Lokasi_ujian_model', 'm');
		$this->form_validation->set_error_delimiters('','');  
		$this->model->CheckAuthorize();
		$this->menu = $this->model->GetMenu($this->kode_menu);
    }

	public function nama_lokasi_ujian_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$check = $this->model->GetSelect("id", $this->table, "nama_lokasi_ujian = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('nama_lokasi_ujian_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function index_get()
	{
		...
		$data = array(
			...
			'content' => 'lokasi_ujian/lokasi-ujian-data',
		);
	...
	public function add_post()
	{
		...
		$data_validation = array(
			'nama_lokasi_ujian' => $this->post('nama_lokasi_ujian'),
			);
		$this->form_validation->set_data($data_validation);
		$this->form_validation->set_rules('nama_lokasi_ujian', 'Lokasi Ujian', 'trim|required|max_length[150]');
		...
		if (form_error('nama_lokasi_ujian') == null) {
			$this->form_validation->set_rules('nama_lokasi_ujian', 'Lokasi Ujian', 'callback_nama_lokasi_ujian_check');
			$this->form_validation->run();
			if (form_error('nama_lokasi_ujian')) {
				$error_validation['nama_lokasi_ujian'] = form_error('nama_lokasi_ujian');
			}else{
				$status_validation .= "1";
			}
		}
	...
	public function update_post()
	{
		...
		$data_validation = array(
			...
			'nama_lokasi_ujian' => $this->model->post('nama_lokasi_ujian'),
			);		
		$this->form_validation->set_data($data_validation);
		...
		if ($myData->num_rows() == 0) {
			...
			if (form_error('nama_lokasi_ujian') == null) {
				$this->form_validation->set_rules('nama_lokasi_ujian', 'Lokasi Ujian', 'callback_nama_lokasi_ujian_check['.$this->post("id").']');
				$this->form_validation->run();
				if (form_error('nama_lokasi_ujian')) {
					$error_validation['nama_lokasi_ujian'] = form_error('nama_lokasi_ujian');
				}else{
