...
class Jenis_peserta extends Rest_Controller {
	
	var $kode_menu = "M002001";
	var $table = "ref_jenis_peserta";
	var $menu;

	function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->model('Jenis_peserta_model', 'm');
		$this->form_validation->set_error_delimiters('','');  
		$this->model->CheckAuthorize();
		$this->menu = $this->model->GetMenu($this->kode_menu);
    }

	public function kode_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$check = $this->model->GetSelect("id", $this->table, "kode = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('kode_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function nama_jenis_peserta_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$check = $this->model->GetSelect("id", $this->table, "nama_jenis_peserta = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('nama_jenis_peserta_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function index_get()
	{
		...
		$data = array(
			...
			'content' => 'jenis_peserta/jenis-peserta-data',
		);
	...
	public function table_post()
	{
        ...
        foreach ($data as $key => $value) {
            ...
            $result['data'][$key] = array(
            	'id' => $this->model->select_data($value->id),
                'kode' => $this->model->select_data($value->kode),
	...
	public function add_post()
	{
		...
		$data_validation = array(
			'kode' => $this->post('kode'),
			'nama_jenis_peserta' => $this->post('nama_jenis_peserta'),
			);
		$this->form_validation->set_data($data_validation);
		$this->form_validation->set_rules('kode', 'Kode', 'trim|required|max_length[10]');
		$this->form_validation->set_rules('nama_jenis_peserta', 'Jenis Peserta', 'trim|required|max_length[150]');
		...
		if (form_error('kode') == null) {
			$this->form_validation->set_rules('kode', 'Kode', 'callback_kode_check');
			$this->form_validation->run();
			if (form_error('kode')) {
				$error_validation['kode'] = form_error('kode');
			}else{
				$status_validation .= "1";
			}
		}
		if (form_error('nama_jenis_peserta') == null) {
			$this->form_validation->set_rules('nama_jenis_peserta', 'Jenis Peserta', 'callback_nama_jenis_peserta_check');
			$this->form_validation->run();
			if (form_error('nama_jenis_peserta')) {
				$error_validation['nama_jenis_peserta'] = form_error('nama_jenis_peserta');
			}else{
				$status_validation .= "12";
			}
		}
	...
	public function update_post()
	{
		...
		$data_validation = array(
			...
			'kode' => $this->model->post('kode'),
			'nama_jenis_peserta' => $this->model->post('nama_jenis_peserta'),
			);		
		$this->form_validation->set_data($data_validation);
		...
		if ($myData->num_rows() == 0) {
			...
			if (form_error('kode') == null) {
				$this->form_validation->set_rules('kode', 'Kode', 'callback_kode_check['.$this->post("id").']');
				$this->form_validation->run();
				if (form_error('kode')) {
					$error_validation['kode'] = form_error('kode');
				}else{
			...
			if (form_error('nama_jenis_peserta') == null) {
				$this->form_validation->set_rules('nama_jenis_peserta', 'Jenis Peserta', 'callback_nama_jenis_peserta_check['.$this->post("id").']');
				$this->form_validation->run();
				if (form_error('nama_jenis_peserta')) {
					$error_validation['nama_jenis_peserta'] = form_error('nama_jenis_peserta');
				}else{
