...
class Jenis_tes extends Rest_Controller {
	
	var $kode_menu = "M002003";
	var $table = "ref_jenis_tes";
	var $menu;

	function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->model('Jenis_tes_model', 'm');
		$this->form_validation->set_error_delimiters('','');  
		$this->model->CheckAuthorize();
		$this->menu = $this->model->GetMenu($this->kode_menu);
    }

	public function nama_jenis_tes_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$check = $this->model->GetSelect("id", $this->table, "nama_jenis_tes = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('nama_jenis_tes_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function index_get()
	{
		...
		$data = array(
			...
			'content' => 'jenis_tes/jenis-tes-data',
		);
	...
	public function table_post()
	{
        ...
        foreach ($data as $key => $value) {
            ...
            $result['data'][$key] = array(
            	'id' => $this->model->select_data($value->id),
                'nama_jenis_tes' => $this->model->select_data($value->nama_jenis_tes),
