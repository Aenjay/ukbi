...
class Dimensi_model extends CI_Model {
	...
	var $table = 'dimensi';
	var $column_order = array(null,null, null, 'nama_dimensi');
	var $column_search = array('nama_dimensi');
	var $order_default = "this.sequence asc";

	private function _get_datatables_query()
	{
		$this->db->select('id, nama_dimensi, allow_detail, allow_edit, allow_delete');
		$this->db->from($this->table." this");
		$this->db->where('this.is_deleted', 0);
		$this->model->like('this.nama_dimensi', $this->model->post('nama_dimensi'));
		$i = 0;
