...
class Users_level extends Rest_Controller {
	
	var $kode_menu = "M999002";
	var $table = "users_level";
	var $menu;

	function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->model('Users_level_model', 'm');
		$this->form_validation->set_error_delimiters('','');  
		$this->model->CheckAuthorize();
		$this->menu = $this->model->GetMenu($this->kode_menu);
    }

	public function name_level_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$check = $this->model->GetSelect("id", $this->table, "name_level = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('name_level_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function index_get()
	{
		...
		$data = array(
			...
			'content' => 'users_level/users-level-data',
		);
	...
	public function table_post()
	{
        ...
        foreach ($data as $key => $value) {
            ...
            $result['data'][$key] = array(
                ...
                'description' => $this->model->select_data($value->description),
                'sequence' => $this->model->select_data($value->sequence),
                'allow_login' => ($value->allow_login == 1) ? "Ya" : "Tidak",
	...
	public function add_post()
	{
		...
		$data_validation = array(
			'name_level' => $this->post('name_level'),
			'description' => $this->post('description'),
			'sequence' => $this->post('sequence'),
			'allow_login' => $this->post('allow_login'),
			);
		$this->form_validation->set_data($data_validation);
		$this->form_validation->set_rules('name_level', 'Nama Level', 'trim|required|min_length[5]|max_length[50]');
		$this->form_validation->set_rules('sequence', 'Sequence', 'trim|required|max_length[11]|numeric');
		$this->form_validation->set_rules('allow_login', 'Allow Login', 'trim|required|max_length[1]|numeric');
		...
		if (form_error('name_level') == null) {
			$this->form_validation->set_rules('name_level', 'Email', 'callback_name_level_check');
			$this->form_validation->run();
			if (form_error('name_level')) {
				$error_validation['name_level'] = form_error('name_level');
			}else{
				$status_validation .= "1";
			}
		}
	...
	public function update_post()
	{
		...
		$data_validation = array(
			...
			'name_level' => $this->model->post('name_level'),
			'description' => $this->model->post('description'),
			'sequence' => $this->model->post('sequence'),
			'allow_login' => $this->model->post('allow_login'),
			);		
		$this->form_validation->set_data($data_validation);
		...
		if ($myData->num_rows() == 0) {
			...
			if (form_error('name_level') == null) {
				$this->form_validation->set_rules('name_level', 'Name Level', 'callback_name_level_check['.$this->post("id").']');
				$this->form_validation->run();
				if (form_error('name_level')) {
					$error_validation['name_level'] = form_error('name_level');
				}else{
