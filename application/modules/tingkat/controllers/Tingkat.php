...
class Tingkat extends Rest_Controller {
	
	var $kode_menu = "M002002";
	var $table = "tingkat";
	var $menu;

	function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->model('Tingkat_model', 'm');
		$this->form_validation->set_error_delimiters('','');  
		$this->model->CheckAuthorize();
		$this->menu = $this->model->GetMenu($this->kode_menu);
    }

	public function nama_tingkat_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$check = $this->model->GetSelect("id", $this->table, "nama_tingkat = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('nama_tingkat_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function add_post()
	{
		...
		$data_validation = array(
			'nama_tingkat' => $this->post('nama_tingkat'),
			'sequence' => $this->post('sequence'),
			);
		$this->form_validation->set_data($data_validation);
		$this->form_validation->set_rules('nama_tingkat', 'Tingkat', 'trim|required|max_length[150]');
		...
		if (form_error('nama_tingkat') == null) {
			$this->form_validation->set_rules('nama_tingkat', 'Tingkat', 'callback_nama_tingkat_check');
			$this->form_validation->run();
			if (form_error('nama_tingkat')) {
				$error_validation['nama_tingkat'] = form_error('nama_tingkat');
			}else{
				$status_validation .= "1";
			}
		}
		if ($status_validation != "1" or $this->form_validation->run() == FALSE) {
			...
			if ($this->db->insert($this->table, $data)) {
				...
				$listDimensi = $this->model->GetSelect("id, nama_dimensi", "dimensi", "is_deleted = '0' order by sequence asc");
				foreach ($listDimensi->result() as $key => $value) {
					$data_insert = array(
						'dimensi_id' => $value->id,
						'tingkat_id' => $insert_id,
	...
	public function update_post()
	{
		...
		$data_validation = array(
			...
			'nama_tingkat' => $this->model->post('nama_tingkat'),
		...
		if ($myData->num_rows() == 0) {
			...
			if (form_error('nama_tingkat') == null) {
				$this->form_validation->set_rules('nama_tingkat', 'Tingkat', 'callback_nama_tingkat_check['.$this->post("id").']');
				$this->form_validation->run();
				if (form_error('nama_tingkat')) {
					$error_validation['nama_tingkat'] = form_error('nama_tingkat');
				}else{
