...
class Jenis_tes_model extends CI_Model {

	var $table = 'ref_jenis_tes';
	var $column_order = array(null,null, null, 'nama_jenis_tes');
	var $column_search = array('nama_jenis_tes');
	var $order_default = "nama_jenis_tes asc";

	private function _get_datatables_query()
	{
		$this->db->select('id, nama_jenis_tes, allow_detail, allow_edit, allow_delete');
		$this->db->from($this->table." this");
		$this->db->where('this.is_deleted', 0);
		$this->model->like('this.nama_jenis_tes', $this->model->post('nama_jenis_tes'));
		$i = 0;
