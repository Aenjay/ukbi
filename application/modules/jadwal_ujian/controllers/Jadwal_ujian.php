...
class Jadwal_ujian extends Rest_Controller {
	
	var $kode_menu = "M003004";
	var $table = "jadwal_ujian";
	var $menu;

	function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->model('Jadwal_ujian_model', 'm');
		$this->form_validation->set_error_delimiters('','');  
		$this->model->CheckAuthorize();
		$this->menu = $this->model->GetMenu($this->kode_menu);
    }

	public function tanggal_check($value, $id='')
	{
		$value = input($value);
		$this->model->where("id !=", input($id));
		$this->db->where('lokasi_ujian_id', $this->input->post('lokasi_ujian_id'));
		$check = $this->model->GetSelect("id", $this->table, "tanggal = '$value' and is_deleted = '0'")->num_rows();
		if ($check > 0 ) {
			$this->form_validation->set_message('tanggal_check', "{field} sudah ada.");
			return false;
		}else{
			return true;
		}
	...
	public function index_get()
	{
		...
		$data = array(
			...
			'content' => 'jadwal_ujian/jadwal-ujian-data',
			'listLokasiUjian' => $this->model->GetSelect("id, nama_lokasi_ujian", "ref_lokasi_ujian", "is_deleted = '0' order by nama_lokasi_ujian asc"),
		);
	...
	public function where_get()
	{
		...
		if ($this->form_validation->run() ==  FALSE) {
			...
			if ($this->get("count") == 1) {
				...
				$this->db->select('this.*, nama_lokasi_ujian');
			}
	...
	public function table_post()
	{
        ...
        foreach ($data as $key => $value) {
            ...
            $result['data'][$key] = array(
            	'id' => $this->model->select_data($value->id),
                'nama_lokasi_ujian' => $this->model->select_data($value->nama_lokasi_ujian),
                'tanggal' => $this->model->select_data($value->tanggal),
                'keterangan' => $this->model->select_data($value->keterangan),
	...
	public function add_post()
	{
		...
		$this->form_validation->set_rules('keterangan', 'Keterangan', 'trim|max_length[250]');
		...
		if (form_error('tanggal') == null) {
			...
			if (form_error('tanggal')) {
				$error_validation['tanggal'] = form_error('tanggal');
			}else{
				$status_validation .= "1";
			}
		}
	...
	public function update_post()
	{
		...
		$data_validation = array(
			...
			'lokasi_ujian_id' => $this->model->post('lokasi_ujian_id'),
			'tanggal' => $this->model->post('tanggal'),
			'keterangan' => $this->model->post('keterangan'),
			);		
		$this->form_validation->set_data($data_validation);
		...
		if ($myData->num_rows() == 0) {
			...
			if (form_error('tanggal') == null) {
				$this->form_validation->set_rules('tanggal', 'Tanggal', 'callback_tanggal_check['.$this->post("id").']');
				$this->form_validation->run();
				if (form_error('tanggal')) {
					$error_validation['tanggal'] = form_error('tanggal');
				}else{
