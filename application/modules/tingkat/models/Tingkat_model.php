...
class Tingkat_model extends CI_Model {
	...
	var $table = 'tingkat';
	var $column_order = array(null,null, null, 'nama_tingkat');
	var $column_search = array('nama_tingkat');
	var $order_default = "this.sequence asc";

	private function _get_datatables_query()
	{
		$this->db->select('id, nama_tingkat, allow_detail, allow_edit, allow_delete');
		$this->db->from($this->table." this");
		$this->db->where('this.is_deleted', 0);
		$this->model->like('this.nama_tingkat', $this->model->post('nama_tingkat'));
		$i = 0;
